...
if "evolution_error" not in st.session_state:
    st.session_state.evolution_error = None
if "raw_evolution_output" not in st.session_state:
    st.session_state.raw_evolution_output = ""
...
def extract_payload(response_text: str):
    # Upgraded regex to be far more tolerant of AI formatting quirks
    ...
    summary_match = re.search(r'```markdown\s*(.*?)\s*```', response_text, re.DOTALL | re.IGNORECASE)
...
if not st.session_state.evolution_triggered:
    with st.spinner("Accessing historical knowledge base. Formulating evolution payload..."):
        ...
        st.session_state.raw_evolution_output = raw_output # Store for debug
        
        if raw_output and "System Offline" not in raw_output:
            new_state, new_code, summary = extract_payload(raw_output)
            
            if new_code and new_state and validate_safety_and_syntax(new_code):
                ...
                st.session_state.pending_update = False
                if not new_code:
                    st.session_state.evolution_error = "EVOLUTION FAILED: Model did not output a valid ```python block."
                elif not new_state:
                    st.session_state.evolution_error = "EVOLUTION FAILED: Model did not output a valid ```json block."
                else:
                    st.session_state.evolution_error = "EVOLUTION FAILED: Model omitted a safety anchor or broke Python syntax."
        else:
            st.session_state.evolution_error = f"API ERROR: {raw_output}"
...
# Persistent Error Display & Debugging
if st.session_state.evolution_error:
    st.error(st.session_state.evolution_error)
    with st.expander("🔍 View Raw Model Output (Debug)", expanded=False):
        st.markdown("If the model truncated the code or failed a safety anchor, review its raw output below:")
        st.code(st.session_state.raw_evolution_output, language="markdown")
